...
En faisant un test sur la normalité de la distibution des varaibles de notre base de donnée, la statistique du test nous indique qu'aucunne des variables n'est distribuée normalement. Aussi,les variables balance, tenure et EstimatedSalary n'ont pas de valeur atypique. Pa ailleurs, on note la présence de valeur atypique dans la distribution des valeurs des variables creditscore et age.
...
On note une absence totale de corrélation entre nos variables quantitatives ce qui est plutôt bien. En effet, l'absence de corrélation nous permet d'éviter le problème de multicolinéarité. Aussi, en effectuant le test d'indépendance de khi2 entre nos différentes variables qualitaives(voir notebook) on conclut que nos variables qualitatives sont indépendantes entre elles car La pvalue de l'ensemble des tests de khi2 étant supérieur au seuil de risque de 5% on conclut qu'il y'a indépendance.
...
Il arrive régulièrement que les jeux de données soient déséquilibrés( ic 79,63% de non churn contre 20,37% de churn).  Le risque est que le modèle prédit simplement tous ou la plupart des points comme la classe majoritaire car les données sont biaisées. En effet, certains modèles, comme les modèles linéaires, ne sont pas performants dans ce type de situation.  
Il y a deux approches pour rééquilibrer les données : Under sampling et Over sampling. Dans cette étude, nous allons utiliser l'approche over sampling basée sur la méthode de SMOTE. En effet, cette méthode est connue pour ça bonne performance, sans perte d'information et surout elle permet de générer un nouveau point qui reste dans l'espace de notre distribution( classe minoritaire ) en sélectionant alétoirement une observation minoritaire initialle et crée ensuite une nouvelle observation entre l'observation minoritaire sélectionner et son plus proche voisin.

Afin de prédire les clients churn, nous avons entraîné plusieurs types d’algorithmes de classification : SVC, LinearSVC, SGDClassifier et Logistic Regresion. Le modèle final est à sélectionner en se basant sur le critère Précision-Rappel car e problème étant de modéliser le phénomène de churn dans la banque donc utilser le rappel  permet de mesure de combien vraiment les résultats pertinents( le churn) sont retournés.
On a procédé aussi à l'optimisation de ces modèles en faisant varier leurs hyperparamètres. Dans notre cas d'usage, ces derniers ont été optimisés à partir d’une validation croisée k-fold en utilisant la méthode Grid search. En effet, cette méthode à l'avantage de trouver les meilleurs hyperparametres en effectuant une recherche exhaustive, son inconvéniant est qu'il est coûteux en temps de calcul car il évalue toutes les combinaisons possibles d'hyperparamètres.
...
# conclusion
 ...
 Dans cette étude nous allons mis en place quatre méthodes SVM afin de modéliser le phénomène de churn.
 Notre meileur modèle parmis l'ensemble des modèles implémenté est le SVC avec une précision de 88.54% et rappel de 92%.
Par la suite, nous avons implémenté une méthode ANN pour modéliser le phenomène de churn; nous avons remarquer que, les clients qui se désabonne sont detectés à 42% ce qui plus faible par rapport au méthode SVM. Ainsi, le meilleur modèle qui permet de prédire avec le phomème de churn est le modèle SVC.
